...
export function useTablaVFuctions() {
    ...
    const actualizarUsuario = async (usuarioActualizado) => {
        try {
            ...
            const response = await axios.patch(urlUA,
                usuarioActualizado,
                {
                    headers: {
                        Authorization: `${token}`,
    ...
    const eliminarUsuario = async (matricula) => {
        try {
            const urlUE = `${BASE_PATH}${RouteUE}`;
            await axios.delete(urlUE, {
                data: { matricula },
                headers: {
                    Authorization: `${token}`,
                },
    ...
    const buscarUsuarioPorMatricula = async () => {
        try {
            const urlUBM = `${BASE_PATH}${RouteUBM}`;
            const response = await axios.post(urlUBM,
