...
const GraficaBarraTurno = () => {
    const BASE_PATH = ENV.BASE_PATH;
    const RouteABPT = ENV.API_ROUTES.ovuhvdfcaQKBemSca10h1niFA7cjpqXAGbEY1cMaNPvPKiWmzAuPAT;
    ...
    const exportToExcel = async () => {
        try {
            ...
            const worksheet = workbook.addWorksheet('Datos de Turno');
    ...
    const fetchData = async () => {
        try {
            if (plantelSeleccionado !== '' && semanaSeleccionada !== '') {
                const urlABPT = `${BASE_PATH}${RouteABPT}`;
                const requestData = {
                    semana: semanaSeleccionada,
                    plantel: plantelSeleccionado,
                };

                const response = await axios.post(urlABPT, requestData, {
                    headers: {
                        Authorization: `${token}`,
                    },
                });
    ...
    const handlePlantelCambio = (event) => {
        ...
        <div>
            <div className="row">
                <div className="col-md-6">
                    <Form.Select onChange={handlePlantelCambio}>
...
export default GraficaBarraTurno;
